#!/bin/bash
# Pushes the latest s3://$DEPLOY_BUCKET/trading-bot/app.zip onto an ALREADY
# RUNNING instance via SSM Run Command (no SSH needed) and restarts the
# service. Run deploy/package_and_upload.sh first to actually update what's
# in S3. The instance must be running (during the 6am-8pm window, or start
# it manually with: aws ec2 start-instances --instance-ids $INSTANCE_ID).
set -euo pipefail
